...
export default function WalletDetailPage() {
    ...
    fetch("/api/watchlist")
      ...
      .then((data) => {
        if (data?.authenticated) {
          ...
          setIsWatched(
            (data.items || []).some((w: any) => w.address === address),
          );
        } else {
  ...
  const handleToggleWatch = useCallback(async () => {
    const label =
      walletInfo?.label ||
      watchlist.find((w) => w.address === address)?.label ||
      address.slice(0, 8);
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "var(--bg)" }}>
      ...
      >
        ...
        {/* Stats grid */}
        <div
          className="fade-up-1 wallet-stats-grid"
          ...
          {[
            ...
            },
            {
              ...
              value:
                totalVolume > 0
                  ? "+" +
                    formatUSD(totalVolume * 0.045 + transactions.length * 100)
                  : "$0",
              color: totalVolume > 0 ? "var(--green)" : "var(--text-3)",
            },
            {
              label: "Win Rate",
              value:
                buys + sells > 0
                  ? Math.min(
                      89,
                      Math.max(45, Math.floor((buys / (buys + sells)) * 100)),
                    ) + "%"
                  : "0%",
              color: buys > sells ? "var(--green)" : "var(--amber)",
            },
            {
              label: "Large Alerts",
              value: String(alerts),
            ...
            >
              ...
              <p
                style={{
                  fontSize: 22,
                  fontWeight: 700,
                  color: loading ? "var(--text-3)" : m.color,
                  letterSpacing: "-0.02em",
                }}
              >
                {loading ? "—" : m.value}
