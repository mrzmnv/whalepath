...
export default async function AlertsAdminPage() {
  ...
  const totalAlerts = await prisma.transaction.count({
    where: { isAlert: true },
  });
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--text-3)", fontSize: 14, marginBottom: 24 }}>
        Large transactions flagged as alerts. Total:{" "}
        <strong style={{ color: "var(--text-1)" }}>{totalAlerts}</strong>
      </p>
        ...
        >
          ...
          >
            <thead>
              ...
              >
                <th style={{ ...cell, fontSize: 12, color: "var(--text-2)" }}>
                  Wallet
                </th>
                <th style={{ ...cell, fontSize: 12, color: "var(--text-2)" }}>
                  Type
              ...
              {alertTxs.map((tx) => (
                ...
                >
                  ...
                  <td style={{ ...cell }}>
                    <span
                      style={{
                        color: typeColor[tx.type] || "var(--text-2)",
                        fontWeight: 600,
                  ...
                  <td
                    style={{ ...cell, fontWeight: 600, color: "var(--text-1)" }}
                  >
