export default function SettingsAdminPage() {
  ...
  const row = (label: string, value: string, ok: boolean) => (
    <tr key={label} style={{ borderBottom: "1px solid var(--border-strong)" }}>
      <td
        style={{ padding: "12px 16px", fontFamily: "monospace", fontSize: 13 }}
      >
        {label}
      </td>
      <td
        style={{ padding: "12px 16px", fontSize: 13, color: "var(--text-3)" }}
      >
        {value}
      </td>
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--text-3)", fontSize: 14, marginBottom: 24 }}>
        Environment variable status. Edit values in your deployment environment
        (Coolify).
      </p>
